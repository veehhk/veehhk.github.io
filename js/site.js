(function ($) {
     ...
     */
    function getAge() {
        const birthDate = new Date(1990, 5, 15, 23, 54, 0); // Replace with the actual date of birth
        const currentDate = new Date();

        let years = currentDate.getFullYear() - birthDate.getFullYear();
        let months = (currentDate.getMonth() + 1) - birthDate.getMonth();
        ...
        return years + " yrs, " + months + " mons, " + days + " dys, " + hours + " hrs, " + minutes + " mins, " + seconds + " secs";
    }
    ...
    function findDayOrNight() {
        var present_time = new Date().getHours();
        if (present_time >= 18 || present_time < 6) {
            return true;
        } else {
            return false;
        }
    ...
    $(function() {
        ...
        $('#liPlay').on('click', function() {
            //openFullscreen();
            // $('#yogi').removeClass('floating');
            // $('#yogi').removeClass('animate-up');
            $('#yogi').removeClass('animated');
            $('#yogi').addClass('animate-up');
            document.getElementById('bgm').currentTime = 0;
        ...
        function setCssClassBasedOnDayAndNightTime() {

        }

        setInterval(() => {
            let current_time = new Date();
            if(current_time.getHours() == 18 & current_time.getMinutes() == 0 & current_time.getSeconds() == 0) {
                Paint();
            }
            if(current_time.getHours() == 6 & current_time.getMinutes() == 0 & current_time.getSeconds() == 0) {
                Paint();
            }

            if(!playerOn) {
                if($('#liPlay').css('background-color') == 'rgba(99, 99, 99, 0.2)') {
                    $('#liPlay').css("background-color", "");
                } else {
                    $('#liPlay').css("background-color", "rgba(99, 99, 99, 0.2)");
                }
            } else {
                $('#liPlay').css("background-color", "");
            }

            $('#spanAgeInYearsMonthsDays').html(getAge());
            
        }, 1000);

        // Sanskrit Quotes Font Rotator
        // setInterval(() => {
        //     if(!fontToggle) {
        //         $('.quote-box').removeClass('sharada');
        //         $('.quote-box').addClass('uttara');
        //         $('#quoteText').html('इदञ्छरीरमशाश्वतम् ।<br>अहङ्गमिष्यामि शाश्वतंपदम् ॥');
        //         fontToggle = true;
        //     } else {
        //         $('.quote-box').removeClass('uttara');
        //         $('.quote-box').addClass('sharada');
        //         $('#quoteText').html('𑆅𑆢𑆚𑇀𑆗𑆫𑆵𑆫𑆩𑆯𑆳𑆯𑇀𑆮𑆠𑆩𑇀 𑇅<br>𑆃𑆲𑆕𑇀𑆓𑆩𑆴𑆰𑇀𑆪𑆳𑆩𑆴 𑆯𑆳𑆯𑇀𑆮𑆠𑆁𑆥𑆢𑆩𑇀 𑇆');
        //         fontToggle = false;
        //     }
        // }, 5000);

        // Bubles mouse follower
    ...
    $(window).load(function () {
        ...
        if (grid_seector.length > 0) {
            ...
            // Isotope Filter
            // var filter1 = $('#project-filter');
            // if (filter1.length > 0) {
            //     var filter_btn = filter1.find('button');
            ...
            // var filter2 = $('#portfolio-filter');
            // if (filter2.length > 0) {
            //     var filter_btn = filter2.find('button');
            //     var filter_btn_first = $('.filter-btn-group button:first-child');
